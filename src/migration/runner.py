...
class MigrationRunner:
    ...
    def _filter_safe_operations(self, diff: List[tuple]) -> List[tuple]:
        ...
        if dropped_count > 0:
            chacc_logger.warning(
                f"Safe mode: Skipped {dropped_count} destructive operations"
            )
    ...
    def _diff_to_migrations(self, diff: List[tuple]) -> List[Dict]:
        ...
        for op in diff:
            ...
            if op_type == 'add_table':
                ...
                table_name = op[1].name
            elif op_type in ('add_column', 'drop_column', 'modify_type', 'modify_nullable', 'modify_default'):
        ...
        try:
            ...
            if backup_path and os.path.exists(backup_path):
                ...
                try:
                    ...
                    chacc_logger.critical(f"CRITICAL: Migration failed AND restore failed: {restore_error}")
                    raise RuntimeError(
    ...
    def _apply_operation(self, op: Operations, op_type: str, details: tuple):
        ...
        if op_type == 'add_table':
            ...
            table_name, column, existing_type, new_type = details[1], details[2], details[3], details[4]
            op.alter_column(table_name, column.name, type_=new_type)
        
        elif op_type == 'modify_nullable':
            table_name, column, existing_nullable, new_nullable = details[1], details[2], details[3], details[4]
            op.alter_column(table_name, column.name, nullable=new_nullable)
        
        elif op_type == 'modify_default':
            table_name, column, existing_default, new_default = details[1], details[2], details[3], details[4]
