                ...
                with zipfile.ZipFile(chacc_path, 'r') as zip_ref:
                    with zip_ref.open('module_meta.json') as meta_file:
                        ...
                        if meta_data.get('name') == module_name:
                            chacc_logger.info(f"Found matching .chacc file for module '{module_name}': {f}")
                            return chacc_path
            except Exception:
                continue
        ...
        try:
            with zipfile.ZipFile(chacc_filepath, 'r') as zip_ref:
                ...
                try:
                    ...
                    chacc_logger.warning(f"No module_meta.json found in {chacc_filename}, using filename as module name")
...
def unzip_modules(modules_to_process: List[Tuple[str, str, float, bool]], existing_records: Dict[str, ModuleRecord], db):
    """
...
def sync_database_with_filesystem(chacc_to_module_name: Dict[str, str], existing_records: Dict[str, ModuleRecord], db):
    """
