...
from chacc_api.utils import PLUGINS_DIR, MODULES_LOADED_DIR, MODULES_UPLOAD_DIR, MODULES_INSTALLED_DIR, DEVELOPMENT_MODE, DEPENDENCY_CACHE_DIR, BACKBONE_REQUIREMENTS_LOCK_FILE, DEPENDENCY_CACHE_FILE, ENABLE_PLUGIN_HOT_RELOAD, BASE_DIR
...
config = {
    ...
    "reload_excludes": [
        ...
        "*.log",
        ".env",
        f"{get_relative_path(PLUGINS_DIR)}/" if "/" in get_relative_path(PLUGINS_DIR) else get_relative_path(PLUGINS_DIR) if not DEVELOPMENT_MODE and ENABLE_PLUGIN_HOT_RELOAD else "",
    ]
}
